...
```{r}
familias_extpobreza_distrito <- left_join(familias_extrema_pobreza, distritos, "ds_nome")
```

#### 1. Quantas famílias em situação de extrema pobreza existiam em São Paulo em 2023?
```{r}
df_23 <- 
  familias_extpobreza_distrito %>% 
  filter(ano == "2023")
...
familias_extpobreza_distrito %>% 
  ...
  labs(x = "Ano", y = "Quantidade de famílias")
```
...
"De acordo com os dados do ObservaSampa, em 2023 metade dos Distritos de São Paulo, no máximo, **5610** famílias em situação de extrema pobreza".  Esse valor foi encontrado a partir da mediana - ou segundo quartil -, medida de centralidade que indica exatamente o valor que separa a amostra na metade. Assim, esse valor indica justamente o complemento adequado à frase.